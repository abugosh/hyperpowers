...
**Delegation model:** This command uses blocking subagent dispatch. The lead reads a pre-planned task list and dispatches a fresh executor subagent (Sonnet by default, promotable per skills/common-patterns/pipeline-constants.md) per task via the Agent tool. The executor returns a one-liner (DONE:, BLOCKED:, or NEEDS_HELP:). The lead runs two-stage per-task review (spec check + code quality review), then proceeds to the next task. A reviewer subagent validates the assembled whole at the end.
