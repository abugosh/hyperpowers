...
For reviewing someone else's branch, MR, or PR with no bd spec involved: **Peek** (`/hyperpowers:peek`) resolves the target, confirms stated intent and prior review state (recon), fans out three parallel judgment lenses (CODE, ARCHITECTURE, DELIVERY), then synthesizes a verdict-led aimed-vs-achieved report — architect-altitude summary over file:line evidence — and offers an optional draft comment that carries a re-peek marker and a follow-up line (Critical: fix and re-review; Important: fix and self-certify), an optional gated forge approval on APPROVE and APPROVE WITH CHANGES when the reviewer is not the author, plus optional gated mechanical fixes applied at the worktree and pushed as additive commits once the user approves the exact diff. No bd epic required.
