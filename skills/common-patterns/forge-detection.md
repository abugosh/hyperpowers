...
| C12 | `gh pr review <number> --approve` with no `--body` is doc-verified (body optional for approve) but not run; the only body-required error text the research confirms belongs to the `--request-changes` form (`body cannot be blank for request-changes review`), so whether the approve form can refuse for a missing body, and with what message, is unconfirmed | On any error naming a missing or blank body, report the message verbatim and treat the approval as not submitted — never match on a specific string, and never re-run with a `--body` the user did not approve at the consumer's gate (peek Step 7: approval failures are reported, never retried). Settled by: one approve on a test PR with no body; if the body proves mandatory, the command above gains a fixed one-line `--body` and this row closes |
