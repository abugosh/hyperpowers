...
- Post: the write commands cited from `skills/common-patterns/forge-detection.md`. On GitLab, that file's C2 caveat applies — the `note create` form first, the legacy form second. At rung 2 or 3, hand the reply text over as copy-paste instead of posting.

**Thread resolution belongs to the reviewer.** Post the reply and leave the thread open. Resolving your own threads takes the reviewer's decision away from them and breaks the next run's work queue.
