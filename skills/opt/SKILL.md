...
- Post: one reply into each source thread, using the **Thread reply** write commands in `skills/common-patterns/forge-detection.md` — never that file's top-level comment form, which opens a conversation the reviewer's thread never sees and which, on GitHub, overwrites the previous reply instead of adding one. Its caveats govern the fallbacks: C10 for the reply forms themselves, and C2 where a rejected reply has to go out as a top-level comment after all — `note create` first, the legacy form second. At rung 2 or 3, hand the reply text over as copy-paste instead of posting.
