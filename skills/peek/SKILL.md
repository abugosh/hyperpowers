...
| 7 | Fix + comment gate | Two phases: elect a comment, fixes, and — on APPROVE / APPROVE WITH CHANGES at rung 1 when the identity read shows the caller is not the author — a forge approval (A); then approve the exact diff, comment text, delivery target, and approve command (B); at most one comment per run, never before the user approves its exact text; comment opens with the verdict, carries the follow-up line under it (Critical: fix and re-review; Important: fix, no re-review), and ends with the `<!-- peek: <sha> -->` marker; delivery order is fixes, comment, approval |
...
**Approval eligibility (lead-owned, before phase A).** The approve election is offered only when ALL of these hold: the verdict — the Step 6 verdict, or the re-derived verdict when applied fixes survive — is `APPROVE` or `APPROVE WITH CHANGES`; the run is at rung 1; the target's state is open; the identity read (`skills/common-patterns/forge-detection.md`, Identity — cite it, never restate the command) returned a login, and that login differs from the target's own author login (the Author fields table in the same file); and RECON's Prior Review approvals do not already list that login — at the reviewed tip on GitHub, at all on GitLab, whose approvals carry no SHA (caveats C14 and C15). When any condition fails the offer is withdrawn and phase A says which one and why, in one line — `forge approval not offered: identity read failed (<error>)`, `self-peek: you are the author (<login>)`, `already approved at this tip` (on GitLab, where no SHA exists to name, `already approved` — C14), `target is <merged|closed>`, `rung <2|3>: no forge` — never silently. Self-peek is comment-only on both forges even where the forge would accept the approval (GitLab Free does). The target's state and its author login come from the Read commands `skills/common-patterns/forge-detection.md` lists — the `gh pr view --json` read carries `state` and `author`, the `glab mr view -F json` read likewise; cite that file's Read commands and its Author fields, never restate them — and the caller's login comes from its Identity subsection. The lead runs those reads at eligibility time, because RECON's return carries neither the target's state nor the caller's login.
...
**Approval failures — reported, never retried.** GitHub's refusal of a self-approval (`Can not approve your own pull request`), GitLab's bare 401, and GitLab's 409 on a `--sha` mismatch — the branch moved, and the posture is a rejected push's: say so, do not chase the new tip — each end the approval step with the forge's output quoted verbatim. The comment stays posted, the branch stays as delivered, no second comment is posted, and the approval is never re-submitted in the run. Every message and status here is the one `skills/common-patterns/forge-detection.md` documents (Approve, caveats C12-C15) — cite it, never restate it.

**Fallbacks — the review still lands, the fixes do not.** Each of these leaves the author's branch exactly as the review found it, and each falls back to the report-only comment: the original findings, the pre-fix verdict, and the reviewed tip as the marker SHA. Approval eligibility on a fallback is read against the pre-fix verdict, and an approval elected at phase A rides with that report-only comment — its exact command, target, and identity line shown beside the comment text, and submitted only after the comment posts.
