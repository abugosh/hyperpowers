...
| 7 | Fix + comment gate | Two phases: elect a comment, fixes, and — on APPROVE / APPROVE WITH CHANGES at rung 1 when the identity read shows the caller is not the author — a forge approval (A); then — only when applied fixes survive — approve the exact diff, comment text, delivery target, and approve command at (B), the comment text and the approve command being approved beside each other at (A) on every other path, the approval withdrawn unsubmitted when the pre-approve tip read or a failed delivery finds the branch moved past the reviewed tip; at most one comment per run, never before the user approves its exact text; comment opens with the verdict, carries the follow-up line under it (Critical: fix and re-review; Important: fix, no re-review), and ends with the `<!-- peek: <sha> -->` marker; delivery order is fixes, comment, approval, the tip read gating that last step |
...
**Approval eligibility (lead-owned, before phase A).** The approve election is offered only when ALL of these hold: the verdict — the Step 6 verdict, or the re-derived verdict when applied fixes survive — is `APPROVE` or `APPROVE WITH CHANGES`; the run is at rung 1; the target's state is open; the identity read (`skills/common-patterns/forge-detection.md`, Identity — cite it, never restate the command) returned a login, and that login differs from the target's own author login (the Author fields table in the same file); and RECON's Prior Review approvals do not already list that login — at the reviewed tip on GitHub, at all on GitLab, whose approvals carry no SHA (caveats C14 and C15). When any condition fails the offer is withdrawn and phase A says which one and why, in one line — `forge approval not offered: identity read failed (<error>)`, `self-peek: you are the author (<login>)`, `already approved at this tip` (on GitLab, where no SHA exists to name, `already approved` — C14), `target is not open (<state>)`, `rung <2|3>: no forge` — never silently. One line in that set fires later than phase A: `branch moved since the reviewed tip (<reviewed-tip> -> <current-tip>)`, stated at delivery time whenever the target's tip has left the tip the comment's marker names — on the two moved-branch fallbacks below (a rejected or non-fast-forward push, and a failed compare-and-swap), and on any other path where the pre-approve tip read defined next comes back with a different SHA. That read is what fills in `<current-tip>` on every path that states this line. Eligibility clearing at phase A is never a promise the approval submits; it holds only while the tip the marker names is still the target's tip. Self-peek is comment-only on both forges even where the forge would accept the approval (GitLab Free does). The target's state and its author login come from the Read commands `skills/common-patterns/forge-detection.md` lists — the `gh pr view --json` read carries `state` and `author`, and the `glab mr view -F json` read carries `state` and `author` too; that file names each forge's state vocabulary, and any value other than its open value means the target is not open; cite that file's Read commands and its Author fields, never restate them — and the caller's login comes from its Identity subsection. The lead runs those reads at eligibility time, because RECON's return carries neither the target's state nor the caller's login.

**The pre-approve tip read — defined here, cited on every path below that submits.** Every path that submits an approval reads the target's current tip first — after the comment posts, immediately before the approve call; the only other time the read runs is at a failed delivery on the two moved-branch fallbacks, to name `<current-tip>` for the moved-tip line. On a remote target that read is `git ls-remote --heads origin <source-branch>`, the same idiom Step 2 runs for the delivery-remote check, read here for the SHA it prints; on a local target it is `git -C <target-repo> rev-parse refs/heads/<branch>`. Compare what comes back with the tip the comment's marker names — the reviewed tip on every path where no fix was delivered, the post-fix tip when fixes were delivered. On a match, submit the approval. On a mismatch, withdraw it unsubmitted and state the moved-tip line, `<current-tip>` being the SHA the read just returned. The two moved-branch fallbacks below are an exception to the gating, not to the read: the failed push or failed compare-and-swap has already proved the move and nothing is submitted there, so the read only names their `<current-tip>`. Without this read an approval binds to whatever the branch carries at the moment of the call, and GitHub's approve names no SHA (`skills/common-patterns/forge-detection.md`, Approve — cite it, never restate it) — an author's push mid-review would be approved silently.
...
- **No fixes elected** (phase A is the only phase): draft the three-part comment here from the Step 6 report, show the user that exact text, and post it on their approval — the comment path is unchanged. When approval was also elected, show the exact approve command, the target, and the identity line ("you are <login>; the author is <login>") together with the comment text; on approval, post the comment first, then run the pre-approve tip read and submit the approval only on a match.
...
Then post the comment under the rules above, run the pre-approve tip read against the tip the marker names, and submit the approval only on a match. Delivery order is fixed — fixes, comment, approval, the read standing between the last two as the approval's precondition — so a failure at any point leaves everything before it delivered and everything after it undone, and says so.

**Approval failures — reported, never retried.** GitHub's refusal of a self-approval (`Can not approve your own pull request`), GitLab's bare 401, and GitLab's 409 on a `--sha` mismatch — the branch moved, and the posture is a rejected push's: say so, do not chase the new tip — each end the approval step with the forge's output quoted verbatim. The comment stays posted, the branch stays as delivered, no second comment is posted, and the approval is never re-submitted in the run. A moved branch caught BEFORE submission is a withdrawal, not a failure: the moved-branch fallbacks below withhold the approval on their failed delivery, and the pre-approve tip read withholds it on every other path. What reaches this paragraph is the narrow race that read cannot close — the branch moving in the moment between the read and the approve call — which GitLab's `--sha` 409 catches and GitHub's SHA-less approve does not. Every message and status here is the one `skills/common-patterns/forge-detection.md` documents (Approve, caveats C12-C15) — cite it, never restate it.

**Fallbacks — the review still lands, the fixes do not.** Each of these leaves the author's branch exactly as the review found it, and each falls back to the report-only comment: the original findings, the pre-fix verdict, and the reviewed tip as the marker SHA. Approval eligibility on a fallback is read against the pre-fix verdict, and what an approval elected at phase A then does splits on whether the branch is known to have moved. On the three fallbacks that leave the reviewed tip standing — no applied fix survives, a cross-fork MR, and a decline at phase B — the approval rides with that report-only comment: its exact command, target, and identity line shown beside the comment text, and submitted after the comment posts and only when the pre-approve tip read still finds the reviewed tip at the target. On the two moved-branch fallbacks — a rejected or non-fast-forward push, and a failed compare-and-swap — it is withdrawn unsubmitted with the `branch moved since the reviewed tip (<reviewed-tip> -> <current-tip>)` line — no gating read is needed there, the failed delivery being the proof — because the branch now carries a tip the review never read: GitLab's approve binds to `--sha` and would refuse it, while GitHub's carries no SHA at all and would approve the new tip silently (`skills/common-patterns/forge-detection.md`, Approve — cite it, never restate it). The comment still posts; only the state change is withheld.
...
1. **Never auto-post, never auto-push, never auto-approve.** A draft comment goes out only after the user approves the exact text, and a fix commit reaches the author's branch only after the user approves the exact diff, comment, and delivery target at Step 7's phase B. A forge approval reaches the forge only after the user approves the exact approve command, target, and identity line — at phase B on a fix run, beside the comment text on every other path — only after the comment is posted and after the pre-approve tip read finds the tip the marker names still at the target, and never on a self-peek, at rung 2/3, or once the branch is known to have moved past the reviewed tip.
...
- [ ] Approval (if eligible and elected): identity read cited from forge-detection, caller's login differed from the author's, target open, no existing approval at the tip; the exact command, target, and identity line approved by the user at the gate that showed them (phase B on a fix run); submitted only after the comment posted and after the pre-approve tip read matched the tip the marker names — a mismatch withdrew it unsubmitted with the moved-tip line instead; any refusal quoted verbatim and never retried; on self-peek, rung 2/3, or an identity failure the withdrawal was stated at phase A, and on a moved-branch fallback — a rejected push or a failed compare-and-swap — the approval was withdrawn unsubmitted with the moved-tip line stated (Step 7)
