---
name: reviewer
description: "Use this agent as a subagent to verify implementation against a bd epic spec. The lead dispatches the reviewer when the executor reports all success criteria met. It applies Google Fellow SRE scrutiny and returns a structured verdict (APPROVED or GAPS FOUND) without flooding the lead's context with implementation details. Examples: <example>Context: Lead's executor sent a Task Completion Report for all tasks. Lead wants to verify before finishing. user: 'Executor reports all criteria met for bd-t4i. Dispatch the reviewer.' assistant: 'I will dispatch the reviewer agent as a subagent to verify the implementation.' <commentary>The reviewer is dispatched as a subagent via the Agent tool (no team_name). It reads the epic, reviews all closed tasks, runs automated checks, and returns a verdict. If GAPS FOUND, the lead sends gap details to a fresh executor for fixes.</commentary></example> <example>Context: Lead received GAPS FOUND from the reviewer and a fresh executor fixed the issues. Now the lead wants to re-verify. user: 'The executor fixed the gaps. Re-run the reviewer.' assistant: 'I will dispatch the reviewer again to verify the fixes.' <commentary>The reviewer can be dispatched multiple times. Each dispatch is a fresh review — it reads the epic and all tasks from bd, not from prior context.</commentary></example>"
model: sonnet
